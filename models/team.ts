...
export const createTeam = async (param: {
  ...
  await addTeamMember(team.id, userId, Role.SUPER_ADMIN);
...
export async function isTeamAdmin(userId: string, teamId: string) {
  ...
  return teamMember.role === Role.ADMIN || teamMember.role === Role.SUPER_ADMIN;
}
...
export const getTeamMember = async (userId: string, slug: string) => {
  return await prisma.teamMember.findFirstOrThrow({
    where: {
      ...
      role: {
        in: ['SUPER_ADMIN','GERENTE', 'ADMIN', 'MEMBRO'],
      },
    },
    include: {
      team: true,
